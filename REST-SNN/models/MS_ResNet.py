...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        # x B C H W
        '''encoding'''
        # 初始化 mask 为 None (防止 GAC=False 时报错)
        mask = None
    ...
    def forward(self, x):
        # x = add_dimention(x, self.T) # 注释掉这一行
        
        # --- [修复] 使用 Out-of-place 操作来扩展时间维度 ---
        # 1. unsqueeze(1) 返回一个新的 Tensor，而不是修改原 Tensor
        x = x.unsqueeze(1) 
        # 2. 在时间维度复制 T 次
...
if __name__ == '__main__':
    # 测试一下是否工作正常
    model = msresnet18(num_classes=10, using_GAC=True)
    model.T = 4
    ...
    print(f"Output shape: {y.shape}") # Should be [Batch, Time, Classes]
    print(f"Mask shape: {mask.shape}") # Should be [Batch, Time, Channels, H, W]
